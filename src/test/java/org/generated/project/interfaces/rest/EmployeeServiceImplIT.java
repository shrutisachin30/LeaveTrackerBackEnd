...
public class EmployeeServiceImplIT {
	...
	public void testLogin() throws Exception {
		...
		Response response = given().contentType(MediaType.APPLICATION_JSON).body(login).auth().basic("demo", "demo")
				.expect().statusCode(200).when().post(baseUrl + "/psa/loginService");
	...
	public void testLoginFail() throws Exception {
		...
		Response response = given().contentType(MediaType.APPLICATION_JSON).body(login1).auth().basic("demo", "demo")
				.expect().statusCode(200).when().post(baseUrl + "/psa/loginService");
	...
	public void testRegister() throws Exception {
		...
		Response response = given().contentType(MediaType.APPLICATION_JSON).body(register).auth().basic("demo", "demo")
				.expect().statusCode(200).when().post(baseUrl + "/psa/registerEmployee");

		String actualResult = response.body().asString();

		assertThat(response.body().asString())
				.isEqualTo("{\"statusMsg\":\"Registration is Successful\",\"statusCode\":\"201\"}");
	}

	@Test
	public void testRegister_useralreadyexist() throws Exception {
		Response response = given().contentType(MediaType.APPLICATION_JSON).body(employee).auth().basic("demo", "demo")
				.expect().statusCode(200).when().post(baseUrl + "/psa/registerEmployee");

		String actualResult = response.body().asString();

		assertThat(response.body().asString())
				.isEqualTo("{\"statusMsg\":\"User Already Exists\",\"statusCode\":\"500\"}");
	}
}